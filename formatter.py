...
class RouteFormatter:
    ...
    def format_route_output(self, routes, start_name, end_name):
        if not routes:
        ...
        while route_idx <= len(routes):
            ...
            if len(routes) > 1:
                output.append(
                    f" OptimalRoute.CH | Route {route_idx} von {len(routes)}"
                )
            else:
            ...
            if hours > 0:
                time_str = (
                    f"{hours} Stunde{('n' if hours > 1 else '')}, "
